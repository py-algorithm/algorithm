...
function solution(n, m, k, grid) {
  ...
  const visited = Array.from({ length: n }, () =>
    Array.from({ length: m }, () => Array.from({ length: k }).fill(0))
  );
  ...
  while (q.length) {
    ...
    for (const [dr, dc] of dn) {
      ...
      if (cell === "1" && cnt < k && !visited[nr][nc][cnt + 1]) {
        q.push([nr, nc, cnt + 1]);
        visited[nr][nc][cnt + 1] = visited[row][col][cnt] + 1;
      }
      // if is not wall
      else if (cell === "0" && !visited[nr][nc][cnt]) {
        q.push([nr, nc, cnt]);
        visited[nr][nc][cnt] = visited[row][col][cnt] + 1;
      }
    }
  }
