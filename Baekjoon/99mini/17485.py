...
for row in range(1, n):
    for col in range(m):
        if col - 1 >= 0:
            dp[row][col][0] = min(dp[row - 1][col - 1][1], dp[row - 1][col - 1][2]) + metrix[row][col]

        dp[row][col][1] = min(dp[row - 1][col][0], dp[row - 1][col][2]) + metrix[row][col]
        
        if col + 1 < m:
            dp[row][col][2] = min(dp[row - 1][col + 1][0], dp[row - 1][col + 1][1]) + metrix[row][col]
